...
func initDB(filename string) (*sql.DB, error) {
	...
	var database *sql.DB
	{
		log.Informf("opening database file %q ...", filename)
		...
		if nil == database {
			log.Errorf("FAILURE — could not open database file %q: nil database", filename)
			return nil, erorr.Errorf("dbsrv: nil database wen opening SQLite database file %q", filename)
		}

		log.Informf("success — opened database file %q", filename)
	}

	{
		err := initTables(database)
		if nil != err {
			log.Errorf("FAILURE — problem initializing tables in database file %q", filename)
			return database, err
		}
	}
